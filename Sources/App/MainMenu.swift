...
enum MainMenu {
    ...
    private static func viewMenu() -> NSMenuItem {
        submenu("View") { menu in
            ...
            menu.addItem(.separator())
            add(menu, "Toggle Inspector", #selector(AppDelegate.toggleInspector(_:)), "3", [.command, .option])
            menu.addItem(.separator())
            add(menu, "Write", #selector(AppDelegate.showWriteMode(_:)), "1", [.command, .control])
            add(menu, "Board", #selector(AppDelegate.showBoardMode(_:)), "2", [.command, .control])
            menu.addItem(.separator())
